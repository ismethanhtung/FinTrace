...
- Round 4 progress:
  - Added tests cho `binanceService`, `aiProviderService`, `openrouterService`.
  - Coverage services tăng rõ ở nhóm `aiProviderService` và `openrouterService`; `binanceService` đã có baseline nhưng vẫn cần bổ sung thêm nhánh fetch/error cho các endpoint còn lại.
