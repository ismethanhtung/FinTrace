...
import { aiProviderService, ChatMessage } from "../services/aiProviderService";
...
export const useAIChat = ({
            ...
            const next = prev.map((s) =>
                s.id === id
                    ? {
                          ...
                          messages: [],
                          updatedAt: Date.now(),
                          title: `New chat · ${s.symbol.replace("USDT", "")}`,
                      }
                    : s,
            );
            saveSessions(next);
    ...
    const sendMessage = useCallback(
        async (userText: string): Promise<void> => {
            ...
            const systemContent =
                systemPromptTemplate.replace("{CONTEXT}", contextSummary) +
