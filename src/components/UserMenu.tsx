...
export const UserMenu = () => {
  ...
  const menuItems = [
    ...
    { group: 'Analysis', items: [
      { icon: Database, label: 'Data Streams', href: '/data' },
      { icon: Terminal, label: 'Query Engine', href: '/query' },
      { icon: Cpu, label: 'AI Insights', href: '/ai' },
    ]},
    { group: 'Account', items: [
          ...
          <>
            ...
            >
              <div className="p-3 border-b border-main">
                <div className="text-[13px] font-semibold">John Doe</div>
                <div className="text-[11px] text-muted">john.doe@fintrace.io</div>
              </div>

              <div className="max-h-[400px] overflow-y-auto thin-scrollbar py-1">
                {menuItems.map((group, idx) => (
                  <div key={idx} className="py-1">
                    <div className="px-3 py-1 text-[10px] font-bold text-muted uppercase tracking-wider">{group.group}</div>
                    {group.items.map((item, i) => (
                      ...
                      >
                        <item.icon size={14} className="mr-2.5 text-muted" strokeWidth={1.5} />
                        {item.label}
                      </Link>
                    ))}
                  </div>
              ...
              <div className="p-1 border-t border-main bg-secondary/50">
                <button className="w-full flex items-center px-3 py-2 text-[12px] text-rose-500 hover:bg-rose-500/10 rounded-md transition-colors">
                  <LogOut size={14} className="mr-2.5" strokeWidth={1.5} />
                  Sign Out
                </button>
              </div>
            </motion.div>
