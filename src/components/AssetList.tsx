...
import { ArrowUpRight, ArrowDownRight, List, ChevronDown, Search, Clock, Flame } from 'lucide-react';
...
const AssetRow = ({
  ...
  >
    <div className="flex items-center space-x-3">
      ...
      <div>
        <div className="flex items-center space-x-1.5">
          <span className={cn('text-[12px] font-semibold', isSelected && 'text-accent')}>
            {asset.symbol}
          </span>
          {badge}
        </div>
    ...
    <div className="text-right">
      <div className="text-[12px] font-mono font-medium">${priceFmt(asset.price)}</div>
      <div
        className={cn(
...
export const WatchlistDropdown = () => {
  ...
  const handleSelect = useCallback(
    (id: string) => {
      ...
      // Push to recents
      setRecents(prev => {
        const next = [id, ...prev.filter(s => s !== id)].slice(0, MAX_RECENTS);
        saveRecents(next);
        return next;
      });
    },
                ...
                {hasQuery ? (
                  ...
                  <>
                    ...
                    {filteredAssets.length === 0 ? (
                      <div className="p-8 text-center text-muted text-[12px]">No coins found</div>
                    ) : (
