...
- point3 can be empty if not needed`;
    ...
    const callGroq = async (payload: Record<string, unknown>) => {
        ...
        if (!res.ok) {
            ...
            (error as Error & { status?: number; body?: string }).status =
                res.status;
            (error as Error & { status?: number; body?: string }).body =
                errText;
            throw error;
        }
    ...
    const parseDelimitedFallback = (
        ...
        const parts = cleaned
            ...
            .map((p) => p.trim())
            .filter(Boolean);
        const shortTitle = (parts[0] || article.title)
            .replace(/\s+/g, " ")
            .trim()
            .substring(0, 70);
        ...
        ).includes(maybeSentiment as "positive" | "negative" | "neutral")
        ...
        return {
            ...
            summary: normalizeSummary(
                points.length ? points : fallbackSummary(),
            ),
        };
    };
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
        try {
            ...
            const sentiment = (
                ["positive", "negative", "neutral"] as const
            ).includes(parsed.sentiment as "positive" | "negative" | "neutral")
            ...
            return {
                shortTitle: (parsed.shortTitle || article.title)
                    .replace(/\s+/g, " ")
                    .trim()
                    .substring(0, 70),
                sentiment,
                ...
                try {
                    const textFallback = await callGroq({
                        ...
                        messages: [
                            {
                                role: "system",
                                content: textFallbackSystemPrompt,
                            },
                            { role: "user", content: userContent },
                        ],
                        max_tokens: 280,
    ...
    console.warn(
        "[market-news] AI enrich fallback:",
        article.title,
        fallbackErrorMsg,
    );
    return {
...
export async function GET(request: Request) {
    ...
    if (
        ...
        return NextResponse.json(
            ...
            },
            {
                headers: {
                    "Cache-Control":
                        "public, s-maxage=10800, stale-while-revalidate=300",
                },
            },
        );
    }
    ...
    try {
        ...
        // Priority: user-provided key first, then platform key from env/AWS.
        const groqKeyPromise = userProvidedGroqKey
            ? Promise.resolve(userProvidedGroqKey)
            : getGroqApiKey().catch(() => {
                  throw new Error(
                      "Groq API key is missing. Add your key in Settings or configure the platform GROQ_API_KEY/AWS secret.",
                  );
              });
