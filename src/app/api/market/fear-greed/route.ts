...
type FearGreedChartResponse = {
    data?: {
        dataList?: FearGreedPoint[];
        dialConfig?: Array<{ start?: number; end?: number; name?: string }>;
        historicalValues?: {
            now?: FearGreedPoint;
            yesterday?: FearGreedPoint;
            lastWeek?: FearGreedPoint;
            lastMonth?: FearGreedPoint;
            yearlyHigh?: FearGreedPoint;
            yearlyLow?: FearGreedPoint;
        };
    };
    status?: {
        timestamp?: string;
...
const FEAR_GREED_URL = "https://api.coinmarketcap.com/data-api/v3/fear-greed/chart";
...
function normalizeWindow(searchParams: URLSearchParams) {
    ...
    const days = Math.max(7, Math.min(365, safeInt(searchParams.get("days")) ?? DEFAULT_DAYS));
...
export async function GET(request: NextRequest) {
    ...
    const upstream = `${FEAR_GREED_URL}?start=${start}&end=${end}`;
    ...
    try {
        ...
        if (!res.ok) {
            throw new Error(`Fear & Greed upstream error: ${res.status}`);
        }

        const payload = (await res.json()) as FearGreedChartResponse;
        ...
        const points = dataList
            .map((point) => {
                ...
                return {
                    ...
                    btcPrice: Number(point.btcPrice ?? NaN),
                    btcVolume: Number(point.btcVolume ?? NaN),
                };
            })
        ...
        return NextResponse.json(
            {
                ...
                points,
                dialConfig: payload.data?.dialConfig ?? [],
