...
interface MarketContextType {
  ...
  /** Spot assets (top 50 USDT pairs by volume) */
  spotAssets: Asset[];
  /** Futures assets (top 50 USD-M perpetual USDT pairs by volume) */
  futuresAssets: Asset[];
...
export const MarketProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchSpotAssets = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load spot market data');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const fetchFuturesAssets = useCallback(async () => {
    try {
      ...
      const topUSDT = tickers
        ...
        .sort((a, b) => parseFloat(b.quoteVolume) - parseFloat(a.quoteVolume))
        .slice(0, 50);
      setFuturesAssets(topUSDT.map(binanceService.transformFuturesTicker));
    } catch (err) {
      console.error('[MarketProvider] Failed to fetch futures assets:', err);
    } finally {
